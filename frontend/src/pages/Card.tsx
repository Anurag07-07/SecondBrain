...
const Card = ({title,link,tags,type,_id,description,getCard}:ICard) => {
  ...
  useGSAP(() => {
    ...
    useEffect(() => {
        // Twitter's widget object should be available globally now
  ...
  return (
    <div ref={cardRef} className=" card-container lg:relative   lg:flex lg:flex-col lg:flex-wrap lg:gap-y-3  lg:w-[25vw] lg:justify-center lg:items-start lg:rounded-4xl   lg:p-10 lg:bg-gradient-to-bl from-gray-300 via-darkgrey-100  ">
      <div><span>Title: </span>{title}</div>
        ...
        <div className=" lg:font-extralight">{description ? <div><span>Description: </span>{description}</div> :
            <span></span>}</div>
        ...
        <button onClick={() => handleDeleteCard(_id)}
                className=" lg:absolute lg:top-5 lg:right-8 lg:hover:bg-red-600 lg:transition-all lg:duration-500 lg:rounded-2xl lg:p-2">{<IoIosTrash size={20}></IoIosTrash>}</button>
    </div>
  )
}
